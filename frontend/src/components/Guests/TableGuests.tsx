...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/guests/guestsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureGuestsCols';
...
const TableSampleGuests = ({
  ...
  useEffect(() => {
    if (guestsNotify.showNotification) {
      notify(guestsNotify.typeNotification, guestsNotify.textNotification);
    }
  }, [guestsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/guests/guests-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {guests && Array.isArray(guests) && !showGrid && (
        <ListGuests
          guests={guests}
          loading={loading}
...
export default TableSampleGuests;
