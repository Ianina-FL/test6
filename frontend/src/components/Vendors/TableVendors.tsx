...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/vendors/vendorsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureVendorsCols';
...
const TableSampleVendors = ({
  ...
  useEffect(() => {
    if (vendorsNotify.showNotification) {
      notify(vendorsNotify.typeNotification, vendorsNotify.textNotification);
    }
  }, [vendorsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/vendors/vendors-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {vendors && Array.isArray(vendors) && !showGrid && (
        <ListVendors
          vendors={vendors}
          loading={loading}
...
export default TableSampleVendors;
