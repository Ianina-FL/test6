...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/venues/venuesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureVenuesCols';
...
const TableSampleVenues = ({
  ...
  useEffect(() => {
    if (venuesNotify.showNotification) {
      notify(venuesNotify.typeNotification, venuesNotify.textNotification);
    }
  }, [venuesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/venues/venues-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {venues && Array.isArray(venues) && !showGrid && (
        <ListVenues
          venues={venues}
          loading={loading}
...
export default TableSampleVenues;
